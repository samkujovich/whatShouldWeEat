...
class RestaurantService: RestaurantServiceProtocol {
    ...
    func fetchRestaurants(near location: CLLocationCoordinate2D, radius: Double, excludedCuisines: [String] = []) {
        print("🍽️ Fetching restaurants near: \(location.latitude), \(location.longitude) with radius: \(radius)m")
        isLoading = true
        errorMessage = nil
        ...
        print("   - Radius: \(radius) meters (\(radius/1609.34) miles)")
            ...
            .sink(
                receiveCompletion: { [weak self] completion in
                    ...
                    if case .failure(let error) = completion {
                        ...
                        print("✅ Restaurant API request completed successfully")
                    }
                },
                receiveValue: { [weak self] response in
                    print("📊 Received \(response.results.count) restaurants from API")
                    ...
                    print("🍽️ Filtered to \(self?.restaurants.count ?? 0) restaurants after excluding cuisines")
                    print("📍 Calculated distances for all restaurants")
                }
            )
